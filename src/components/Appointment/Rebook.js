...
class Rebook extends Component {
  constructor(props) {
    ...
    let payload = {
      ...
      client: {
        id: this.props.navigation.state.params.ApptInfo.client.id,
      },
    };
    ...
    return new Promise((resolve, reject) => {
      GetServiceInfo(
        ...
        response => {
          response.then(data => {
            if (
              data.technicianServices.find(
                technician => technician.technician.id == technicianId,
              )
            ) {
              return resolve(
                ServiceGenerator(
            ...
            <List style={{marginHorizontal: 20}}>
              ...
              {this.state.payload.visitTechnicians.map((item, index) => (
                <Fragment key={index}>
                  ...
                  {this.state.payload.visitTechnicians.length - 1 != index && (
                    <View
                      style={{
                      ...
                      <View style={{width: '80%'}}>
                        <View
                          ...
                          <View
                            ...
                            <Text style={{color: '#666'}}>
                              <FontAwesome color="#666" name="circle" /> Price:
                              $ {(item.chargeAmount * 1).toFixed(2)}
                            </Text>
                          </View>
                        </View>
                      </View>
